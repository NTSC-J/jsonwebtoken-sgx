...
int SGX_UBRIDGE(SGX_NOCONVENTION, u_thread_wait_event_ocall, (int* error, const void* tcs, int timeout));
#endif
...
int SGX_UBRIDGE(SGX_NOCONVENTION, u_thread_setwait_events_ocall, (int* error, const void* waiter_tcs, const void* self_tcs, int timeout));
#endif
...
int SGX_UBRIDGE(SGX_NOCONVENTION, u_readdir64_r_ocall, (int* error, void* dirp, struct dirent64_t* entry, struct dirent64_t** result));
#endif
